 ...
 */
	export default {
		...
		props: {
			/** @type {boolean} 是否允许过期补签 */
			isReplenishSign: {
				type: Boolean,
				default: false
			},
			/** @type {boolean} 是否填满日历（前后空格显示上月/下月日期） */
			isFullCalendar: {
				type: Boolean,
				default: true
			},
			/** @type {string} 初始显示的年月，格式："2022-01" */
			yearMonth: {
				type: String,
				default: new Date().getFullYear()+'-'+ new Date().getMonth() + 1
			},
			/** @type {Array} 已签到的数据源，格式：["2022-01-05","2022-01-06"] */
			dataSource: {
				type: Array,
				default: () => {
					return []
